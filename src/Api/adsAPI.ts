...
export const delAds=(token:string,id:number)=>{
  return new Promise<{ data: Payads[] }>((resolve,reject) =>
    axios.delete(MY_server+"ads/"+id,{ headers: {
      'Authorization': `Bearer ${token}`
    }}).then(res => resolve({ data: res.data })).catch(error => {reject(error)})
  );
}
...
export const delPool=(token:string,id:number)=>{
  return new Promise<{ data: Payads[] }>((resolve,reject) =>
    axios.delete(MY_server+"pool/"+id,{ headers: {
      'Authorization': `Bearer ${token}`
    }}).then(res => resolve({ data: res.data })).catch(error => {reject(error)})
  );
}
...
export const delPayAds=(token:string,id:number)=>{
  return new Promise<{ data: Payads[] }>((resolve,reject) =>
    axios.delete(MY_server+"adsPay/"+id,{ headers: {
      'Authorization': `Bearer ${token}`
    }}).then(res => resolve({ data: res.data })).catch(error => {reject(error)})
  );
}